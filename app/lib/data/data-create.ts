...
import {createHitPoints} from '@/lib/data/hp-data';
import {createArmourClass} from '@/lib/data/ac-data';
import {createSpeed} from '@/lib/data/speed-data';
...
const createCategories = async (char:any, char_id:string) => {
   ...
   const _hitPoints = await createHitPoints(char.hit_points, char_id);
   const _armourClass = await createArmourClass(char.armour_class, char_id);
   const _speed = await createSpeed(char.speed, char_id);
